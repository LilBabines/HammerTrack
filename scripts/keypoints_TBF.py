...
Extract per-track skeletal keypoints (head, center-of-mass, two articulations,
...
# Module-level state for the optional ffplay preview (set inside main()).
_FFPLAY_PROC = None
...
def extract_keypoints_basic(binary_mask):
    """Extract head (mid-cephalofoil), COM, art1, art2, tail from a mask."""
    ...
    # Tail = farthest contour point from COM
    ...
    # Head: widest cross-section on the head side, weighted by distance to COM
    best_score = 0
    ...
    # Articulations at 1/3 and 2/3 along the COM->tail axis
    articulations = []
    for frac in [1 / 3, 2 / 3]:
...
def compute_angles_basic(kp):
    """Compute one articular angle (head-com-tail) + 7 directional angles."""
...
def process_track_mask(args):
    """Compute keypoints + angles for a track (runs in a worker thread)."""
...
def send_to_display(canvas):
    ...
    if not _DISPLAY_ALIVE or _FFPLAY_PROC is None:
        return
    if _FFPLAY_PROC.poll() is not None:
        _DISPLAY_ALIVE = False
        return
    _FFPLAY_PROC.stdin.write(cv2.resize(canvas, (_DISP_W, _DISP_H)).tobytes())
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Extract keypoints and angles from per-track JSON files using SAM2, "
            "and render a skeleton display video alongside per-track keypoint CSVs."
        ),
    ...
    p.add_argument("--csv-dir", required=True,
                   help="Output directory for per-track keypoint CSVs.")

    # --- SAM2 ---
    p.add_argument("--sam2-checkpoint", default="sam2.1_hiera_base_plus.pt",
    ...
    p.add_argument("--device", default="cuda",
                   help="Torch device for SAM2 inference.")

    # --- Inference / batching ---
    p.add_argument("--chunk-size", type=int, default=100,
                   help="Number of frames processed per SAM2 chunk.")
    p.add_argument("--mask-threshold", type=float, default=0.85,
                   help="Min mask confidence; below that, re-prompt with the bbox.")
    p.add_argument("--max-tracks-per-batch", type=int, default=12,
                   help="Max tracks segmented together per SAM2 batch.")
    p.add_argument("--num-workers", type=int, default=min(8, os.cpu_count() or 4),
                   help="Threads for the keypoint post-processing pool.")
    p.add_argument("--chunk-frames-dir", default="/tmp/sam2_chunk",
                   help="Scratch directory used for SAM2 chunk symlinks.")

    # --- Rendering ---
    p.add_argument("--mask-alpha", type=float, default=0.45,
                   help="Alpha used for the mask overlay.")
    p.add_argument("--contour-thickness", type=int, default=4,
                   help="Thickness of the mask contour.")
    p.add_argument("--graph-seconds", type=int, default=5,
                   help="Time window (s) shown in the angle graph.")
    p.add_argument("--graph-height", type=int, default=400,
                   help="Height of the side angle graph in pixels.")
    p.add_argument("--no-graph", action="store_true",
                   help="Disable the side graph in the rendered video.")
    p.add_argument("--no-display", action="store_true",
                   help="Do not open a live ffplay preview window.")
...
def main():
    global _FFPLAY_PROC, _DISPLAY_ALIVE, _DISP_W, _DISP_H
    ...
    out_video_dir = os.path.dirname(os.path.abspath(args.output_video))
    if out_video_dir:
    ...
    sample_frame = cv2.imread(os.path.join(args.frames_dir, f"{0:06d}.jpg"))
    if sample_frame is None:
        sys.exit(f"Could not read first frame from: {args.frames_dir}/000000.jpg")
    h, w = sample_frame.shape[:2]
    print(f"Frame size: {w}x{h}, {total_frames} frames, {fps} fps")

    cap_orig = cv2.VideoCapture(args.video)
    ...
    writer = cv2.VideoWriter(
        args.output_video, cv2.VideoWriter_fourcc(*"mp4v"), fps, (canvas_w, h)
    )

    # --- Live display (optional) ---
    _DISP_W, _DISP_H = int(canvas_w * 0.35), int(h * 0.35)
    if show_video:
        _FFPLAY_PROC = subprocess.Popen(
            ["ffplay", "-f", "rawvideo", "-pixel_format", "bgr24",
             "-video_size", f"{_DISP_W}x{_DISP_H}", "-framerate", str(fps),
    ...
    reprompt_count = 0
    max_buffer = int(fps * args.graph_seconds)
    angle_buffers = {tid: [] for tid in tracks}
    latest_kp = {tid: None for tid in tracks}

    for chunk_start in range(0, total_frames, args.chunk_size):
        chunk_end = min(chunk_start + args.chunk_size, total_frames)
        chunk_len = chunk_end - chunk_start
        print(f"\n=== Chunk frames {chunk_start}-{chunk_end - 1} ===")

        # Symlinks for the chunk (SAM2 expects a contiguous frame folder)
        if os.path.exists(args.chunk_frames_dir):
            shutil.rmtree(args.chunk_frames_dir)
        os.makedirs(args.chunk_frames_dir)
        for i, gi in enumerate(range(chunk_start, chunk_end)):
            src = os.path.join(args.frames_dir, f"{gi:06d}.jpg")
            dst = os.path.join(args.chunk_frames_dir, f"{i:06d}.jpg")
        ...
        if not active_tids:
            for i in range(chunk_len):
                gi = chunk_start + i
                frame = cv2.imread(os.path.join(args.frames_dir, f"{gi:06d}.jpg"))
        ...
        batches = [active_tids[i:i + args.max_tracks_per_batch]
                   for i in range(0, len(active_tids), args.max_tracks_per_batch)]
        ...
        for batch_idx, batch_tids in enumerate(batches):
            ...
            inference_state = predictor.init_state(
                video_path=args.chunk_frames_dir,
            ...
            with torch.autocast(args.device, dtype=torch.bfloat16):
                ...
                for local_idx, obj_ids, mask_logits in predictor.propagate_in_video(inference_state):
                    ...
                    for i, sam2_id in enumerate(obj_ids):
                        ...
                        if mask_score < args.mask_threshold:
                            det = tracks[tid]["det_by_frame"].get(gi)
                            already_prompted = (
                                det is not None
                        ...
                        if binary_mask.shape[:2] != (h, w):
                            binary_mask = cv2.resize(
                                binary_mask, (w, h), interpolation=cv2.INTER_NEAREST
                            )
        ...
        for local_idx in range(chunk_len):
            ...
            for tid, (binary_mask, mask_score) in frame_tracks.items():
                if binary_mask.sum() > 0:
                    ...
                    frame = cv2.addWeighted(overlay, args.mask_alpha, frame, 1 - args.mask_alpha, 0)
                    contours, _ = cv2.findContours(binary_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    cv2.drawContours(frame, contours, -1, tracks[tid]["color"], args.contour_thickness)

            # 2) Keypoints in parallel
    ...
    for f in csv_files.values():
        f.close()
    if _DISPLAY_ALIVE and _FFPLAY_PROC is not None:
        _FFPLAY_PROC.stdin.close()
        _FFPLAY_PROC.wait()
    shutil.rmtree(args.chunk_frames_dir, ignore_errors=True)
    print(f"\nDone! {args.output_video} ({total_frames} frames, {reprompt_count} re-prompts)")
    print(f"Keypoints CSVs in: {args.csv_dir}")
